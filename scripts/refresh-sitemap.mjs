...
const [
  ...
  fetchAll("people","select=id,display_name,role,organization,bio,photo_url,profile_url,source_url,created_at&verification_status=eq.verified&order=display_name.asc"),
...
function personPage(p){
  ...
  if(org)personObj.affiliation={"@type":"Organization","name":org}; if(img)personObj.image=img; if(p.profile_url)personObj.sameAs=[p.profile_url];
