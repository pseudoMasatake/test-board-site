...
const [
  ...
  fetchAll("people","select=id,display_name,role,organization,bio,overview,birth_date,sex,photo_url,profile_url,source_url,created_at&verification_status=eq.verified&order=display_name.asc"),
...
function personPage(p){
  const url="https://saibanwatch.github.io/people/"+encodeURIComponent(p.id)+".html", role=roles[p.role]||"関係者", org=txt(p.organization,160), overview=txt(p.overview||p.bio,900), birth=p.birth_date||"", sex=({male:"男性",female:"女性",other:"その他"})[p.sex]||"未確認", img=safe(p.photo_url);
